...
cd /opt/3bm && docker compose build --no-cache openaec-reports-rs
...
sleep 5
if curl -sf https://report-rs.3bm.co.nl/api/health > /dev/null; then
    echo "✓ Rust API is live: $(curl -s https://report-rs.3bm.co.nl/api/health)"
else
