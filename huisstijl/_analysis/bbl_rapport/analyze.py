...
# Analyseer eerste 4 pagina's (cover, colofon, content) + laatste pagina (backcover)
...
for page_idx in pages_to_analyze:
    ...
    # Drawings/vectors (rectangles, lines)
    print("\n--- DRAWINGS ---")
    drawings = page.get_drawings()
    for d in drawings[:20]:  # Limit
        items_desc = []
        for item in d["items"]:
        ...
        stroke_str = f"stroke=({stroke[0]:.2f},{stroke[1]:.2f},{stroke[2]:.2f})" if stroke else "no-stroke"
        if items_desc:
