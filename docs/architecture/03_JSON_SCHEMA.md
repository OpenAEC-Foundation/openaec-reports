# JSON Input Schema — OpenAEC

## Overzicht

OpenAEC gebruikt een andere template-structuur dan Customer.
De documenten zijn flow-based (lopende tekst met secties) in plaats van
fixed-page layouts.

> ⚠️ **STATUS:** OpenAEC templates draaien op V1 (engine.py) en V2 (renderer_v2.py).
...
OpenAEC gebruikt een geneste `sections` structuur met content blocks:
...
Wanneer een tenant naar V3 migreert:
1. Maak `tenants/{tenant_slug}/page_types/` aan
2. Definieer page_types per rapport type
3. Maak template YAML's (welke pagina's in welke volgorde)
4. Verplaats stationery naar `tenants/{tenant_slug}/stationery/`
5. Schrijf `data_transform` regels voor tenant-specifieke content blocks
