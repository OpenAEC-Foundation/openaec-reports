...
- [ ] `https://report.3bm.co.nl/api/health` → OK
- [ ] `/api/generate/template` endpoint werkt (Symitech BIC)
...
De TemplateEngine (V3) werkt voor Symitech. Nu ook 3BM rapporten migreren.

- [ ] T3.1 — 3BM page_type YAML's aanmaken (voorblad, content, colofon, achterblad)
- [ ] T3.2 — 3BM template YAML's (structural, daylight, building_code)
...
- [ ] T4.9 — API key prefix: `3bm_k_` → `oaec_k_` (in `auth/api_keys.py`)
