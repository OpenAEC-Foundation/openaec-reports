...
class TestGenerateApiKey:
    ...
    def test_key_has_prefix(self):
        """Gegenereerde key begint met 3bm_k_ prefix."""
        key = generate_api_key()
        assert key.startswith("3bm_k_")

    def test_key_has_sufficient_length(self):
        """Key is lang genoeg (prefix + 64 hex chars = 70 chars)."""
        key = generate_api_key()
        assert len(key) == 6 + 64  # "3bm_k_" + 32 bytes hex
...
class TestHashApiKey:
    ...
    def test_different_keys_different_hashes(self):
        ...
        assert hash_api_key("3bm_k_aaa") != hash_api_key("3bm_k_bbb")
...
class TestApiKeyDB:
    ...
    def test_create_returns_key_and_plaintext(self, _ensure_test_users):
        ...
        assert plaintext.startswith("3bm_k_")
    ...
    def test_get_by_key_unknown_returns_none(self):
        ...
        db = get_api_key_db()
        result = db.get_by_key("3bm_k_doesnotexist123456")
...
class TestApiKeyAuth:
    ...
    def test_invalid_api_key_returns_401(self, client):
        ...
        r = client.get(
            "/api/templates",
            headers={"X-API-Key": "3bm_k_ongeldige_key_hier"},
        )
...
class TestAdminApiKeyEndpoints:
    ...
    def test_create_api_key(self, admin_client, _ensure_test_users):
        ...
        assert data["plaintext_key"].startswith("3bm_k_")
