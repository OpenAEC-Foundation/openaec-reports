...
class TestPdfExtractor:
    ...
    def test_extract_cover_text(self, raw_pages):
        ...
        for t in p1.texts:
            if "BBL" in t.text and "rapport" in t.text.lower():
                assert "Inter" in t.font or "inter" in t.font.lower()
                assert t.size > 25
                found = True
                break
    ...
    def test_extract_content_text(self, raw_pages):
        ...
        body_texts = [
            t for t in p5.texts
            if "Inter" in t.font and "Book" in t.font and abs(t.size - 9.5) < 1
        ]
        assert len(body_texts) > 0, "Geen Inter-Book 9.5pt tekst op pagina 5"
...
class TestCLI:
    ...
    def test_cli_analyze_brand_creates_files(self, tmp_path):
        ...
        yaml_str = generate_brand_yaml(analysis, "OpenAEC Test", "openaec-test")
        yaml_path = tmp_path / "openaec-test.yaml"
