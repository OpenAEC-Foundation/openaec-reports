...
class TestStationeryEndpoint:
    ...
    @SKIP_NO_STATIONERY
    def test_3bm_cooperatie_complete(self, authenticated_client):
        r = authenticated_client.get("/api/stationery")
        data = r.json()
        brands = data["brands"]
        assert "3bm_cooperatie" in brands
        assert brands["3bm_cooperatie"]["complete"] is True
...
class TestGenerateV2:
    ...
    def test_generate_v2_missing_project(self, authenticated_client):
        r = authenticated_client.post(
            "/api/generate/v2", json={"template": "3bm_cooperatie"}
        )
