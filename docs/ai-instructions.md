...
```http
X-API-Key: oaec_k_your_api_key_here
```

API keys start with the prefix `oaec_k_` and contain 64 hex characters. They are created via the admin panel or admin API. Each key is linked to a user and inherits that user's permissions and tenant.
...
| `author` | string | No | Author (default: `"OpenAEC"`) |
...
| `brand` | string | No | Brand slug (e.g. `"default"`, `"customer"`) |
...
"cover": {
  ...
  "extra_fields": {
    "Reference": "OpenAEC-2026-031-R01",
...
"colofon": {
  ...
  "adviseur_email": "a.jansen@openaec.nl",
  ...
  "disclaimer": "This report is prepared by OpenAEC...",
  "revision_history": [
    {
      "version": "0.1",
      "date": "2026-01-15",
...
BIC (Bouwtechnische Inventarisatiecheck) control table. **Customer tenant module only.**
...
Cost summary table. **Customer tenant module only.**
...
Location data block with client and property information. **Customer tenant module only.**
...
Object description block with key-value fields. **Customer tenant module only.**
...
The `src` field on `image` blocks, the `image` field on `cover`, and `photo_path` fields on Customer blocks all accept three formats:
...
```json
{
  ...
  "cover": {
    "subtitle": "Structural calculation — main load-bearing structure",
    "extra_fields": {
      "Reference": "OpenAEC-2026-042-R01"
    }
  },
