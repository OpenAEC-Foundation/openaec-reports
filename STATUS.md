...
- Template matcht exact de originele 17-pagina Symitech structuur
...
Root cause: Authentik `sub_mode=user_email` → meerdere users met zelfde email kregen zelfde `sub` claim → SSO login als Symitech werd gekoppeld aan admin account.
...
DB state gecorrigeerd: `symitech` user gekoppeld aan Authentik Symitech account via `oidc_subject`.

### Geverifieerd op productie
- [x] Symitech user ziet alleen eigen templates (bic_factuur, bic_rapport)
- [x] Symitech user ziet geen admin panel (role=user)
- [x] 3BM admin accounts werken correct met 3bm_cooperatie tenant
...
- [x] `VITE_API_URL` leeggemaakt (was hardcoded `https://report.3bm.co.nl`)

### SSO werkt weer na deploy
- `report.3bm.co.nl` → redirect naar `report.open-aec.com` verloor het pad
- Oorzaak: frontend had `VITE_API_URL=https://report.3bm.co.nl` gebaked in productie JS
...
- [x] **Templates** — Symitech user ziet alleen eigen templates ✅
- [x] **Admin panel** — Symitech user (role=user) ziet geen admin panel ✅
- [x] **OIDC login** — Symitech en 3BM accounts resolven naar juiste DB users ✅
...
- Symptoom: Symitech user ziet 3BM templates en vice versa
...
- Symptoom: User kon `symitech_bic_factuur` template gebruiken vanuit 3BM tenant
...
- [x] **User fixes op productie** (via admin API):
  - 3BM user: tenant hersteld naar `3bm_cooperatie`
  - jochem user: tenant gezet op `3bm_cooperatie`
  - Nieuwe `symitech` user aangemaakt (local, tenant=symitech)
- [x] **Gepusht naar GitHub** — commit `982461e`
- [ ] **Deploy nog nodig** — `bic_rapport` template nog niet live op server
  - Run `deploy.sh` op server, of: `cd /opt/3bm/bm-reports-api && git pull && cd /opt/3bm && docker compose build --no-cache bm-reports-api && docker compose up -d bm-reports-api`
...
- **Oorzaak:** User heeft `tenant: "default"` (of leeg) in profiel, maar roept template `bic_rapport` aan dat bij tenant `symitech` hoort
...
- **Juiste fix (TODO):** De user moet de correcte tenant-toewijzing krijgen via:
  ...
  2. Of handmatig in de database: `UPDATE users SET tenant='symitech' WHERE ...`
  3. Of de request moet `brand: "symitech"` meesturen
...
- [x] **Deploy** — Gedeployed naar `report.open-aec.com` (redirect van `report.3bm.co.nl`)
...
### Architectuur
- Thin wrapper: frontend identiek aan web, PDF generatie via cloud API (`report.3bm.co.nl`)
- `VITE_API_URL=https://report.3bm.co.nl` in productie build
