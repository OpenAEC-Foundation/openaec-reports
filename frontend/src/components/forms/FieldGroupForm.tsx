          ...
          <div key={field.bind}>
            ...
            {field.input_type === 'textarea' ? (
              <textarea
                ...
                rows={3}
                className="w-full rounded-md border border-oaec-border px-3 py-2 text-sm focus:border-brand-primary focus:outline-none focus:ring-1 focus:ring-brand-primary"
              />
              ...
              <input
                ...
                value={value}
                onChange={(e) => setFlatField(group.key, field.field, e.target.value)}
                className="w-full rounded-md border border-oaec-border px-3 py-2 text-sm font-mono focus:border-brand-primary focus:outline-none focus:ring-1 focus:ring-brand-primary"
              />
              ...
              <div className="space-y-2">
                ...
                />
                {value && (
                  <div className="text-xs text-oaec-text-faint truncate">{value.slice(0, 80)}...</div>
                )}
              </div>
            ) : (
              <input
...
function TableFieldGroupForm({ group }: { group: TableFieldGroup }) {
  ...
  return (
    <div className="space-y-3">
      <div className="overflow-x-auto border border-oaec-border rounded-lg">
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-oaec-bg border-b border-oaec-border">
              <th className="px-2 py-2 text-left text-xs font-semibold text-oaec-text-muted w-8">#</th>
              {group.columns.map((col) => (
                <th
                  key={col.field}
                  className="px-2 py-2 text-left text-xs font-semibold text-oaec-text-muted"
                >
                  {col.header}
                </th>
              ))}
          ...
          <tbody>
            {rows.map((row, rowIdx) => (
              <tr
                key={rowIdx}
                className={`border-b border-oaec-border-subtle ${rowIdx % 2 === 1 ? 'bg-oaec-bg/50' : ''}`}
              >
                <td className="px-2 py-1 text-xs text-oaec-text-faint font-mono">{rowIdx + 1}</td>
                {group.columns.map((col) => (
                  <td key={col.field} className="px-1 py-1">
                    <input
                      ...
                      className="w-full rounded border border-transparent px-1.5 py-1 text-sm hover:border-oaec-border focus:border-brand-primary focus:outline-none focus:ring-1 focus:ring-brand-primary"
                    />
                  </td>
                ))}
      ...
      <button
        ...
        className="flex items-center gap-1.5 rounded-md border border-dashed border-oaec-border px-3 py-2 text-sm text-oaec-text-muted hover:border-brand-primary hover:text-brand-primary transition-colors"
      >
...
export function FieldGroupForm({ groupKey }: { groupKey: string }) {
  ...
  if (!group) {
    return (
      <div className="px-6 py-8 text-center text-oaec-text-faint">
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="sticky top-0 z-10 border-b border-oaec-border bg-oaec-bg-lighter/95 backdrop-blur px-6 py-4">
        <div className="flex items-center gap-3">
          <span className="flex h-7 w-7 items-center justify-center rounded bg-oaec-accent-soft text-sm">
            {group.type === 'table' ? (
          ...
          <h2 className="text-lg font-semibold text-oaec-text">{group.label}</h2>
          {group.type === 'table' && (
            <span className="rounded bg-oaec-hover px-1.5 py-0.5 text-xs text-oaec-text-muted">
              Tabel
            </span>
          )}
        </div>
