...
def main():
    ...
    Usage:
        openaec-report generate --template structural --data project.json --output rapport.pdf
        openaec-report templates --list
        openaec-report validate --data project.json
    """
    parser = argparse.ArgumentParser(
        prog="openaec-report",
        description="OpenAEC Report Generator — Professionele engineering rapporten",
    )
...
def _cmd_create_user(args):
    ...
    if args.username and args.password:
        ...
        from openaec_reports.auth.seed import create_user_interactive
...
def _cmd_create_api_key(args):
    ...
    from openaec_reports.auth.models import UserDB
...
def _cmd_serve(args):
    ...
    print(f"OpenAEC Report API server op http://{args.host}:{args.port}")
    print(f"  Docs: http://localhost:{args.port}/docs")
    uvicorn.run(
        "openaec_reports.api:app",
        host=args.host,
        port=args.port,
        reload=args.reload,
    )
