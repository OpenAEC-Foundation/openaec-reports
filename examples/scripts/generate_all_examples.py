...
Gebruik:
    ...
    python examples/scripts/generate_all_examples.py --only symitech
    python examples/scripts/generate_all_examples.py --only 3bm_cooperatie/structural_report
...
TENANT_DIRS = ["3bm_cooperatie", "symitech", "openaec_foundation"]
...
TEMPLATE_ENGINE_TEMPLATES = {
    "bic_rapport",
    "bic_factuur",
    "symitech_bic_rapport",
    "symitech_bic_factuur",
}
...
def generate_template_engine(example: dict) -> Path:
    ...
    # bijv. "symitech_bic_factuur" → "bic_factuur" (YAML heet bic_factuur.yaml)
    prefix = f"{tenant}_"
...
def main() -> None:
    ...
    parser.add_argument(
        "--only",
        help="Filter op tenant of bestandsnaam (bijv. 'symitech' of '3bm_cooperatie/structural')",
    )
    parser.add_argument(
        "--list",
        action="store_true",
