...
from openaec_reports.auth.dependencies import get_api_key_db, get_user_db, require_admin
...
def _resolve_tenants_base() -> Path:
    ...
    Volgorde:
    1. OPENAEC_TENANTS_DIR environment variable (expliciet pad naar tenants/)
    2. Parent van OPENAEC_TENANT_DIR (als die gezet is, bijv. /app/tenants/openaec → /app/tenants)
    3. Relatief aan source tree (development fallback)

    Returns:
        Path naar de tenants directory.
    """
    tenants_dir = os.environ.get("OPENAEC_TENANTS_DIR") or os.environ.get(
        "OPENAEC_TENANTS_ROOT"
    )
    if tenants_dir:
        return Path(tenants_dir)

    tenant_dir = os.environ.get("OPENAEC_TENANT_DIR")
    if tenant_dir:
