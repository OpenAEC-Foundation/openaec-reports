...
def _make_brand() -> BrandConfig:
    """Laad de 3BM Coöperatie brand voor tests."""
    return BrandLoader().load("3bm_cooperatie")
...
class TestResolveLogoPath:
    def test_resolve_from_brand_logos(self):
        brand = _make_brand()
        path = _resolve_logo_path(brand, "main", "3bm-cooperatie.png")
    ...
    def test_fallback_when_key_not_in_brand(self):
        ...
        path = _resolve_logo_path(brand, "white", "3bm-cooperatie-wit.png")
        assert isinstance(path, Path)
        assert "3bm-cooperatie-wit.png" in str(path)
...
class TestDrawBackcoverPage:
    ...
    def test_draws_contact_info(self):
        ...
        found_contact = any(
            "Zwijndrecht" in str(args) or "3BM" in str(args)
            for args in canvas.drawString.call_args_list
        )
