...
Internet
  │
  ├── report.3bm.co.nl ──→ Caddy (:443) ──→ openaec-reports (:8000)
  └── auth.3bm.co.nl   ──→ Caddy (:443) ──→ authentik-server (:9000)
                                               ├── postgresql
                                               └── redis
```
...
| Record | Type | Waarde |
|--------|------|--------|
| `report.3bm.co.nl` | A | `<server-ip>` |
| `auth.3bm.co.nl` | A | `<server-ip>` |
...
1. Open `https://auth.3bm.co.nl/if/flow/initial-setup/`
...
3. Vul in:
   ...
   - **Redirect URIs:**
     ```
     https://report.3bm.co.nl/auth/callback
...
2. Vul in:
   ...
   - **Launch URL:** `https://report.3bm.co.nl`
...
curl https://report.3bm.co.nl/api/auth/oidc/config

# Authentik discovery
curl https://auth.3bm.co.nl/application/o/openaec-reports/.well-known/openid-configuration
```

Open `https://report.3bm.co.nl` — je zou nu de **"Inloggen via SSO"** knop moeten zien.
...
| Cookie werkt niet cross-domain | `OPENAEC_COOKIE_DOMAIN` moet `.3bm.co.nl` zijn (met punt) |
